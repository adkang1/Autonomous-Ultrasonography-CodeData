...
def computeTSNE(fileName='./for_tsne.pkl'):
    ...
    plt.figure(4)
    plt.scatter(TSNE_hA_3[:, 0], TSNE_hA_3[:, 1],
                c=np.hstack((np.ones((500,)), 2 * np.ones((500,)), 3 * np.ones((500,)))))

    plt.figure(5)
    plt.scatter(TSNE_hA_3[:, 0], TSNE_hA_3[:, 1], c=np.hstack((src_labels, src_labels, trg_labels)))

    plt.figure(6)
    plt.scatter(TSNE_hA_4[:, 0], TSNE_hA_4[:, 1],
                c=np.hstack((np.ones((500,)), 2 * np.ones((500,)), 3 * np.ones((500,)))))
...
def knn(X_test, X_ref, Y_ref, K=5):
    ...
    distance = tf.negative(tf.reduce_sum(input_tensor=tf.abs(tf.subtract(X_ref, X_test[0])), axis=1))  # L1
    values, indices = tf.nn.top_k(distance, k=K, sorted=False)

    nn = []

    for k in range(K):
        nn.append(tf.argmax(input=Y_ref[indices[k]], axis=0))

    nearest_neighbors = nn
    y, idx, count = tf.unique_with_counts(nearest_neighbors)

    preds = tf.slice(y, begin=[tf.argmax(input=count, axis=0)], size=tf.constant([1], dtype=tf.int64))[0]
